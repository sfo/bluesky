    ...
    def __init__(self,name,cmd,cmdargs):
        ...
        if swcircle and self.name[:4]=="SEGM":
            ...
            if success:
                ...
                bs.logger.error("trafgen class Source called for "+name+". Position not found")
    ...
    def __init__(self,name,cmd,cmdargs):
        ...
        if self.name[:4]=="SEGM":
            ...
            if success:
                ...
                bs.logger.error("trafgen class Drain called for "+name+". Position not found")
                self.lat,self.lon = 0.0,0.0
        ...
        if dt>0.0:
            ...
            if random.random() >= chances:
                ...
                if iorig>=0:
                    ...
                    bs.logger.warning("Warning update drain",self.name,"called with no origins present!")
                    hdg = random.random()*360.
                    bs.logger.warning("using random segment",int(hdg+180)%360)
