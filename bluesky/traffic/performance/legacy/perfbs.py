        ...
        for actype in actypes:
            if actype in coeffBS.atype:
                ...
                if not settings.verbose:
                    if not self.warned:
                        bs.scr.echo("Aircraft is using default B747-400 performance.")
                        self.warned = True
                else:
                    bs.scr.echo(f"Flight {bs.traf.id[-1]} has an unknown aircraft type, {actype}, BlueSky then uses default B747-400 performance.")
