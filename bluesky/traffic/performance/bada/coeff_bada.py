...
def init(bada_path=''):
        ...
        with open(releasefile) as f:
            for line in f:
                ...
                if 'Unknown' not in (release_date, bada_version):
                    break
        bs.logger.info('Found BADA version %s (release date %s)' % (bada_version, release_date))
    else:
    ...
    bs.logger.info('%d aircraft entries loaded' % len(synonyms))
    ...
    bs.logger.info('%d unique aircraft coefficient sets loaded' % len(accoeffs))
