...
class TcpSocket:
    ...
    def connectToHost(self, ip, port):
        try:
            ...
            bs.logger.info("Server connected. HOST: %s, PORT: %s" % (ip, port))
        except Exception as err:
            self.is_connected = False
            bs.logger.error("Connection Error: %s" % err)
            pass
    ...
    def disconnectFromHost(self):
        try:
            ...
            bs.logger.error("Disconnection Error: %s" % err)
            pass
    ...
    def receiver(self):
        while True:
            ...
            try:
                ...
                bs.logger.error("Receiver Error: %s" % err)
                time.sleep(1.0)
